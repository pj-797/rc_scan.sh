...
function scanner(){ 	# scanner processes and logging
	...
	# sshpass -p $pass ssh -o LogLevel=QUIET -t $server "nmap --privileged $input -Pn -sS -O -sV" >> $(pwd)/RC_Scan/nmap_${input}  # for possible -sS -O Scan (which require sudo)
...
# remove # from line 183 and add to line 184, or change line 184 to:
# sshpass -p $pass ssh -o LogLevel=QUIET -t $server "nmap --privileged $input -Pn -sS -O -sV" >> $(pwd)/RC_Scan/nmap_${input}
# then, set nmap capablitites in the function: remote_installation, by removing # on line 155 & 156.
# To unset/remove nmap capabilities use line 157.
# WARNING: It will take a long time for each scan, estimated 10-20 mins per scan.
